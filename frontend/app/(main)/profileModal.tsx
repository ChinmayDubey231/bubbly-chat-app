...
const ProfileModal = () => {
  ...
  const onSubmit = async () => {
    ...
    // If there is an avatar, let our smart imageService handle it
    if (avatar) {
      const res = await uploadFileToCloudinary(avatar, "profiles");
      if (res.success) {
        data.avatar = res.data; // This will now be the proper Cloudinary URL
      } else {
        Alert.alert("User", res.msg);
        setLoading(false);
        return;
    ...
    // Now it's safe to send to the database
    updateProfile(data);
  };
